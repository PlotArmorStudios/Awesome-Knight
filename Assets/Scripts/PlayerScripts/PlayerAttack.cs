...
public class PlayerAttack : MonoBehaviour
{
    ...
    void CheckInput()
    {
        ...
        if(m_animator.GetInteger("Atk") == 0) //if not attacking, we are moving, and we have not finished movement
        {
            ...
            if(!m_animator.IsInTransition(0) && m_animator.GetCurrentAnimatorStateInfo(0).IsName("Stand")) //if not in transition and we are standing, we are finished moving
            {
        ...
        if (Input.GetKeyDown(KeyCode.Alpha1))
        {
            if (mana >= 20f)
            {
                _mPlayer.TargetPosition = transform.position; //set targetposition to cuurrent position of the player so player does not move
